...
const useRSwr = (store, path ) => {
  const selector    = useSelector(s => npget(s, path), shallowEqual);
  const setSelector = (newstate) => {
    if (typeof newstate === 'function') {
      store.dispatch({ type: "_"+path+"_", payload: newstate(npget(store.getState(), path)) });
    } else {
      store.dispatch({ type: "_"+path+"_", payload: newstate });
    }
  }
...
const deepupdate = (state, prop, payload) => {
...
const RSReducer = ( state, reducerref, type, payload ) => {
